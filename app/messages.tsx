...
export default function Messages() {
    ...
    useEffect(() => {
        ...
        const initializeChatListSocket = async () => {
            ...
            const socket = io('https://server.riyadah.app/chat-list', {
                auth: { token },
    ...
    const fetchParticipants = async () => {
        try {
            const token = await SecureStore.getItemAsync('userToken');
            const res = await fetch('https://server.riyadah.app/api/chats/participants', {
                headers: {
    ...
    const loadChats = useCallback(async () => {
        ...
        try {
            const token = await SecureStore.getItemAsync('userToken');
            const res = await fetch(`https://server.riyadah.app/api/chats?page=${page}&limit=${pageLimit}`, {
    ...
    const refreshChats = useCallback(async () => {
        ...
        try {
            const token = await SecureStore.getItemAsync('userToken');
            const res = await fetch(`https://server.riyadah.app/api/chats?page=1&limit=${pageLimit}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleConfirmDeleteChat = async (chatId: string) => {
        try {
            const token = await SecureStore.getItemAsync('userToken');
            const res = await fetch(`https://server.riyadah.app/api/chats/delete/${chatId}`, {
                method: 'delete',
                headers: {
        ...
        <GestureHandlerRootView style={styles.container}>
            ...
            <SafeAreaView>
                ...
                >
                    ...
                    {keyword.length < 3 && <BottomSheetScrollView
                        keyboardShouldPersistTaps="handled"
                        contentContainerStyle={{ paddingHorizontal: 15, marginTop: 110, paddingBottom: 140 }}
